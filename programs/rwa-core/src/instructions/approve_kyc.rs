...
use crate::contexts::ApproveKyc;
use crate::constants::*;

// sha256("global:update_kyc_status")[..8]
const UPDATE_KYC_STATUS_DISC: [u8; 8] = [182, 90, 72, 162, 173, 39, 52, 147];

pub fn handler(ctx: Context<ApproveKyc>) -> Result<()> {
    // CPI to compliance-hook::update_kyc_status(is_kyc=true)
    let mut data = Vec::with_capacity(9);
    data.extend_from_slice(&UPDATE_KYC_STATUS_DISC);
    data.push(1u8); // is_kyc = true
    ...
    // thaw the investor's token account
